...
function Login() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    if (!store.getProfile()) {
      store.setProfile({ name: email.split("@")[0] || "Friend", email });
  ...
  return (
    <div className="min-h-screen bg-gradient-soft flex flex-col">
      <div className="mx-auto w-full max-w-md flex-1 flex flex-col px-6 py-8">
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="flex flex-col items-center text-center"
        >
          <div className="rounded-full bg-card shadow-soft p-3 w-20 h-20 flex items-center justify-center animate-float">
            <img src={heroTooth} alt="DentNova" className="w-14 h-14 object-contain" />
          </div>
          <h1 className="mt-4 text-2xl font-bold">Welcome back</h1>
          <p className="text-sm text-muted-foreground mt-1">Sign in to continue your dental journey</p>
        </motion.div>
        ...
        >
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input id="pwd" type={show ? "text" : "password"} required placeholder="••••••••" value={pwd} onChange={(e) => setPwd(e.target.value)} className="pl-10 pr-10 h-12 rounded-xl" />
          ...
          <Button type="button" variant="outline" onClick={submit} className="w-full h-12 rounded-xl border-2 hover:bg-accent/10">
